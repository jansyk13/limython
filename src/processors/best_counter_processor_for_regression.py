...
class BestCounterProcessorForRegression:
    ...
    def __init__(self, node_count):
        ...
        self.predicted_node_counters = [0] * int(node_count)
    ...
    def process(self, request):
        ...
        try:
            ...
            index = self.predicted_node_counters.index(
                min(self.predicted_node_counters))
            # adding payload size to counter
            self.predicted_node_counters[
                index] += request.predicted_payload_size
            self.node_counters[index] += request.payload_size
            log.debug('action=payload-processing index=%s id=%s value=%s predicted=%s' %
                      (index, request.id, request.payload_size, request.predicted_payload_size))
